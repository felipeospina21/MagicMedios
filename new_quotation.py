...
class New_Quotation:
  ...
  def __init__(self, path, prs, title_slide_layout) -> None:
    self.user_path = f"{path}/data"
    self.users = self.get_users()
    self.client = input("Ingrese nombre cliente: ").title()
    self.company = input("Ingrese nombre empresa: ").upper()
    self.user = input(
        f"Ingrese nombre asesor comercial ({', '.join(self.users)}): "
    ).lower()
    self.reference = (
        input("Ingrese referencias a consultar (separadas por coma): ").upper().split(",")
    )

    self.consecutive_path = f"{path}/data/consecutivo.txt"
    self.footer_path = f"{path}/data/data.txt"
    ...
    self.save_path = f"./cotizaciones/cotización_{self.company}.pptm"
  ...
  def set_slides(self):
    ...
    for i in range(0, self.ref_q + 1):
      ...
      text_frame_paragraph(
          tf_footer, f"{self.address} {self.phone} {self.email} {self.web}", 7, False, True
      )
  ...
  def add_header(self, today):
    ...
    tf = txBox.text_frame
    text_frame_paragraph(tf, f"{today.day} {today.strftime('%B')} de {today.year}", 14)
